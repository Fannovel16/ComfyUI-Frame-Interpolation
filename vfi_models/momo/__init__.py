...
def inference(model, img_batch_1, img_batch_2, inter_frames, num_inference_steps):
    ...
    for _ in range(len(remains)):
        ...
        with torch.no_grad():
            prediction, _ = model(x = torch.stack([x0, x1], dim=2), num_inference_steps = num_inference_steps)
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "num_inference_steps": ("INT", {"default": 8, "min": 2, "max": 20}),
            },
            "optional": {
    ...
    def vfi(
        ...
        **kwargs
        ...
        for frame_itr in range(len(frames) - 1): # Skip the final frame since there are no frames after it
            ...
            relust = inference(model, frame_0, frame_1, multipliers[frame_itr] - 1, num_inference_steps)
